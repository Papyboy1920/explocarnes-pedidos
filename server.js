...
// ---------------- Auth de tienda ----------------
if (!STORE_KEY) {
  console.warn("[explocarnes] ⚠️  STORE_KEY no configurado — /tienda abierto (modo desarrollo). Define STORE_KEY en producción.");
}
...
app.post("/api/orders", async (req, res) => {
  ...
  // Explocarnes es 100% móvil: solo delivery en todo Miami.
  if (type !== "delivery") {
    return res.status(400).json({ error: "Solo delivery — Explocarnes es carnicería móvil." });
  }
...
db.init().then(() => {
  app.listen(PORT, () => {
    console.log(`[explocarnes] Servidor corriendo en http://localhost:${PORT}`);
    console.log(`[explocarnes] Clientes: http://localhost:${PORT}/`);
    console.log(`[explocarnes] Tienda:   http://localhost:${PORT}/tienda`);
  });
}).catch((err) => {
  console.error("[explocarnes] ❌ No se pudo inicializar la base de datos:", err.message);
  console.error("[explocarnes] Revisa DATABASE_URL (Postgres) o los permisos del archivo agua-celeste.db (SQLite).");
  process.exit(1);
});
