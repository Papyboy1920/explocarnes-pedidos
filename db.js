...
async function init() {
  if (process.env.DATABASE_URL) {
    ...
    kind = "pg";
    console.log("[explocarnes] DB: Postgres");
  } else {
    const { DatabaseSync } = require("node:sqlite");
    sdb = new DatabaseSync(path.join(__dirname, "explocarnes.db"));
    sdb.exec(SQLITE_SCHEMA);
    kind = "sqlite";
    console.log("[explocarnes] DB: SQLite local (explocarnes.db)");
  }
  ...
  if (!(await kvGet("catalog"))) {
    ...
    console.log("[explocarnes] Catálogo semilla cargado.");
  } else {
    const v = await kvGet("catalog_version");
    if (v !== String(CATALOG_VERSION)) {
      await kvSet("catalog", JSON.stringify(SEED_CATALOG));
      await kvSet("catalog_version", String(CATALOG_VERSION));
      console.log(`[explocarnes] Catálogo re-sembrado (v${v} → v${CATALOG_VERSION}).`);
    }
  }
